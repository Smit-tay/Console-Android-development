...
Now, back to the discussion of how to build.  You see, those steps, and some optional ones we haven't discussed, are tedious to run from the command line, so naturally, it's better to build a script for such things.  The problem with scripts is that they generally aren't compatible across platforms.  Enter, cross platform build systems like Autotools, CMake, Meson, and others.  Those tools were written with the purpose of being cross platform, and are at least partly development language agnostic.  This means you can use them to generate a cross platform build environment which then allows you to build the appropriate build target (I use that word carefully, since it has several meanings).  This usually also includes the ability to rebuild only those things that have been changed in your project.  They also contain the idea of a "Generator" which in many cases will produce a configuration file recognized by various IDEs like Eclipse, or VSCode, or others.  This means, you could continue to use your favourite environment and still build an Android application.   BUT - NO !!!
Instead, Gradle.

Normally re-inventing the wheel is discouraged - unless your wheel is really something special.  If you manage to invent a wheel without a bearing (that actually functions), please use it in preference to one that does have one.  Congrats !   Otherwise, use the wheel.

Or, in the case of Android development, choose the newest, most unrefined build system available at the time, and go for it.  So, that's what they did.  [Gradle ](https://docs.gradle.org/current/userguide/userguide.html) was that choice.  Gradle's initial release (2008) coincided with Android's initial release, the difference is a matter of months at most. Gradle was an unknown, immature product at the time, so why not use it ?!?!?!?   Zero points for guessing what language Gradle was initially developed as a build system for.  Yes, rather than improve Ant(2000), or Maven (2004), or use existing tools like Make (1976), etc. the Android devs chose this java based, java centric build tool to automate the craziness of the Android development cycle - because Androidn wasn't Java (snicker).
